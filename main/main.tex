...
Sociologists have studied social networks to better understand what derives people to form friendship, groups and communities with others. 
...
And last but not least, marketing experts are using social networks to better understand the market, and increase benefit by targeted advertisement.\\
...
% too long
By studying how influence is propagated under two models; \textit{Independent Cascade} and \textit{Linear Threshold} models, they defined the problem of Influence Maximization as finding an initial set of vertices(called \textit{active set} or \textit{seeds}), that will influence the most number of vertices under either models. 
The problem is proven to a \textbf{NP-hard} problem which can be solved using a greedy algorithm. 
Kempe's solution however suffers greatly from scalability issues. 
Even on small graphs, finding the solution takes a long amount of time. \\

In the last decade a new field in computer science has emerged that involves working with massive amount of data. This field, generally referred to as \textbf{Big Data}, focuses on building tools for efficient collection, storage, and processing of data. 

Probably the most notable frameworks that are used widely nowadays are \textit{Hadoop} and \textit{Spark}. Hadoop is an open source project that is based on Google's MapReduce framework introduced by Dean and Ghemawat \cite{dean04}. 
