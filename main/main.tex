...
\textit{Given a size $k$ for the initial seeds, find the set of nodes $A$ that maximizes the function $f$ which is the size of spread throughout the network. }\\ \\
...
\caption{Influence Maximization: Greed Algorithm}
...
\textbf {Page Rank} : Based on the popular Page Rank algorithm by Page et al.\ \cite{page99}, the algorithm was originally used in Google's search engine, to rank websites based on their importance. 
\newpage


\section{Experiments}
\label{sec:experiments}
In this section all the experiments are explained in detail. Experiments include the methods described in previous sections and a number of real and artificially generated data sets.
...
\internalappendix{1}{Giraph Connected Componen source}
\label{app:giraph}
Here is the source for finding the connected components of a graph in Giraph. The code is directly taken from Giraph's repository at:\\
https://github.com/apache/giraph
\scriptsize
\lstinputlisting{codes/giraphcc.java}
\normalsize


\internalappendix{2}{Graphx Connected Component source}
\label{app:graphx}
Here is the source for finding the connected components of a graph in GraphX. The code is directly taken from Graphx's Repository at:\\ https://github.com/apache/spark/tree/master/graphx
\scriptsize
\lstinputlisting{codes/sparkcc.sc}
\normalsize
