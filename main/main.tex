...
Description of what I'm going to do, big data/Hadoop/MapReduce implementations for network influence.
...
\item if superstep == 0 
\begin{itemize}
\item add your own id to list of influencedBy an increase vertex value by one
\item try to activate all neighbors 
\end{itemize}
\item if superstep > 0
\begin{itemize}
\item receive message from every one (message is the id of the vertex)
\item for each message received
\begin{itemize}
\item if it is already in your influecedBy list then vote to halt
\item else 
\begin{itemize}
\item activate all neighbors by using the message
\item add to influecedBy list
\item send a message to source vertex notifying it that you have been activated 
\item vote to halt
\end{itemize}
\end{itemize}
\end{itemize}
